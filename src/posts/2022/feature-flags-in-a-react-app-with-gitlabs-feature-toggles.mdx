...
But how to we use GitLab's feature toggles in a React app? There are three components we need to
work on, these are:
...
their [pricing page](https://about.gitlab.com/pricing/#gitlab-com).

Once logged in to GitLab and while viewing your project (assuming you have the right permissions),
under "Deployments" you should see an option titled _Feature Flags_.
...
will need these two values in a moment when we configure our proxy service. Last bit of
preparation work will be to create a GitLab Access token.
...
Now we have our `API Token`, our `Instance ID` and our `API URL` we're ready to configure our
proxy service. The proxy application sits between GitLab and our React app (the client). It
provides some security and performance benefits as it doesn't expose direct access to
GitLab ensuring the internal feature toggle configuration isn't exposed to the world!

Unleash does offer a free tier "hosted" option instead of having to run your own proxy which you
can find details about on their [pricing page](https://www.getunleash.io/plans).
...
- `UNLEASH_PROXY_SECRETS` - This secret is used in both the client and proxy, and is
used to authorize calls to the proxy from the client app. This can be a comma separated list of
secrets but in this example we're passing a single string `RANDOM_STRING_9vZBzCd2jeqE7JY`.
_Please use a sensible secret in production!_
- `UNLEASH_URL` - This is our API URL from the GitLab configuration dialog.
- `UNLEASH_INSTANCE_ID` - This was the `Instnce ID` from the configuration dialog.
...
All but the `UNLEASH_PROXY_SECRETS` are only used by the proxy with `UNLEASH_PROXY_SECRETS`
being used by both the proxy and the client app. You can find a more detailed breakdown of the
configuration details, their defaults and what they're used for on
...
curl -H "Content-Type: application/json" \
     -H "Authorization: [YOUR SECRET SET IN THE PROXY]" \
     -X GET \
...
can install by running the following in our (already set up) React project:

```shell
npm install @unleash/proxy-client-react
```

With the client installed we can now adjust the app root to add our Provider. With the Unleash
client we pass a `config` property, allowing it can connect to the proxy and
ultimately out GitLab instance. The following snippet shows how we're doing this using
environment variables. We're using [create-react-app](https://create-react-app.dev/docs/adding-custom-environment-variables/) in the following example it will pull our
...
Running this you will see that `example_flag_key` is `false`. Which would be true as we've not
enabled a flag with that name. We don't even have a flag with that name. The `useFlag` function
will always return `false` if a flag is known and disabled, or unknown. To get this component to
show "Enabled" we'll need to create and activate the toggle.

## Adding a feature toggle

Let's add our flag and activate it to see this component go from "Disabled" to "Enabled". Back
in your project's "Feature Flags" dashboard (via `Deployments` / `Feature Flags` ) use the button
in the top right labeled "New Feature flag"
...
Your feature toggle is set to active by default after creating it. Now you can go back
to your React app and once the `refreshInterval` triggers a new fetch, the SDK should show your
new flag as 'Enabled'!

## Activation strategies

Now we already know how to activate toggles for the app as a whole in the admin. A more powerful
feature of GitLab/Unleash comes in the form of activating features for specific users
